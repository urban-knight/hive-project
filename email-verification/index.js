...
module.exports = function(mongoose) {
  ...
  var domain = process.env.HOST||"localhost",
      ...
      confirm_body = fs.readFileSync("./email-verification/html/confirmation.htm", 'utf8'),
      success_body = fs.readFileSync("./email-verification/html/success.htm", 'utf8');
  ...
  var options = {
    ...
    verifyMailOptions: {
      from: 'Hive Gaming <development.hive@gmail.com>',
      subject: 'Подтверждение почты',
      html: confirm_body,
      text: 'Пожалуйста, подтвердите почту по уникальной ссылке: ${URL}'
    },
    ...
    confirmMailOptions: {
      ...
      html: success_body,
      text: 'Указанная при регистрации почта подтверждена и привязана к Вашему аккаунту.'
    },
  ...
  var confirmTempUser = function(url, callback) {
    ...
    TempUser.findOne(query, function(err, tempUserData) {
      ...
      if (tempUserData) {
        ...
        //Custom cast to PassportJS
        var pass = userData[options.passwordFieldName];
        delete userData[options.passwordFieldName];
